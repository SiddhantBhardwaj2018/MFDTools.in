...
export default function businessPlanning() {
  ...
  const [businessFutureCost,setBusinessFutureCost] = useState("");
  ...
  const onSubmitGoalInvestPlan = (e) => {
    ...
    if (
      currentAge.length > 0 &&
      businessCost.length > 0 &&
      businessAge.length > 0 &&
      returnRate.length > 0 &&
      currentInvestmentAmt.length > 0 &&
      inflationRate.length > 0 &&
      inputCalculatorRules.numericRegex.test(currentAge) &&
      inputCalculatorRules.numericRegex.test(businessCost) &&
      inputCalculatorRules.numericRegex.test(businessAge) &&
      ...
      mfToolsService
        .calculateGoalInvestPlan(
          ...
          Number(businessAge.replace(/,/g, "")),
          Number(businessCost.replace(/,/g, "")),
        ...
        .then((res) => {
          ...
          setBusinessFutureCost(inputCalculatorRules.formatINR(res.data.futureCost));
