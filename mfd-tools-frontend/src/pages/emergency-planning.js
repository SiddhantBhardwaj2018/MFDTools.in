...
export default function emergencyPlanning() {
  ...
  const [emergencyFutureCost,setEmergencyFutureCost] = useState("");
  ...
  const onSubmitGoalInvestPlan = (e) => {
    ...
    if (
      currentAge.length > 0 &&
      emergencyCost.length > 0 &&
      emergencyAge.length > 0 &&
      returnRate.length > 0 &&
      currentInvestmentAmt.length > 0 &&
      inflationRate.length > 0 &&
      inputCalculatorRules.numericRegex.test(currentAge) &&
      inputCalculatorRules.numericRegex.test(emergencyCost) &&
      inputCalculatorRules.numericRegex.test(emergencyAge) &&
      ...
      mfToolsService
        .calculateGoalInvestPlan(
          ...
          Number(emergencyAge.replace(/,/g, "")),
          Number(emergencyCost.replace(/,/g, "")),
        ...
        .then((res) => {
          ...
          setEmergencyFutureCost(inputCalculatorRules.formatINR(res.data.futureCost));
    ...
    <>
      ...
      <div style={{ margin: "0 8%" }}>
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block mb-2 text-sm font-medium text-gray-700">
              Expected Emergency Age (Years)
            </label>
            <input
              value={emergencyAge}
              onChange={onChangeEmergencyAge}
              type="text"
              min="0"
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                       focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
          ...
          <div>
            ...
            <input
              value={emergencyCost}
              onChange={onChangeEmergencyCost}
              type="text"
              min="0"
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                       focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
