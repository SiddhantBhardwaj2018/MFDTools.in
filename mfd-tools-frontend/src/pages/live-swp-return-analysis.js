...
export default function businessPlanning() {
  ...
  const [originalInvestAmt, setOriginalInvestAmt] = useState(0);
  const [returnVal, setReturnVal] = useState(0);
  const [totalWithdrawnAmt, setTotalWithdrawnAmt] = useState(0);
  const [withdrawalBalanceSum, setWithdrawalBalanceSum] = useState(0);
  ...
  const onChangeinitialInvestmentAmount = (e) => {
  ...
  const onChangeWithdrawalPercent = (e) => {
    if (inputCalculatorRules.numericRegex.test(e.target.value)) {
  ...
  const triggerSwpAnalysisPull = () => {
    setDisplayResult("none");
    console.log(withdrawalPercent);
    if (selectedSchemes.length > 0 && withdrawalPercent > 0) {
      setLoading(true);
      setCalcError(false);
      let lumpsumAmt = (initialInvestmentAmount + "").replace(/,/g, "");
      mfToolsService
        .calculateSWP(
          investmentDate,
          lumpsumAmt,
        ...
        .then((res) => {
          ...
          if ((res.data != undefined) & (res.data != null)) {
            setMonthlyWithdrawal(res.data.monthlyWithdrawal);
            setOriginalInvestAmt(res.data.originalInvestAmt);
            setReturnVal(res.data.return);
            setTotalWithdrawnAmt(res.data.totalWithdrawnAmt);
            setWithdrawalBalanceSum(res.data.withdrawalBalanceSum);
          } else {
  ...
  const onSearchSwpAnalysisPull = (e) => {
    e.preventDefault();
    triggerSwpAnalysisPull();
  };
  ...
  useEffect(() => {
    if (
      ...
      (new Date(investmentDate) < new Date(withdrawalDate))
    ) {
      triggerPullSchemeList();
    }
  }, [fundHouse, schemeType, investmentDate, withdrawalDate]);
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={onChangeinitialInvestmentAmount}
              type="text"
              min="0"
          ...
          <div>
            ...
            <input
              ...
              onChange={onChangeWithdrawalPercent}
              type="text"
              min="0"
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                       focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
          ...
          <div className="md:col-span-2 flex justify-center mt-4">
            <button
              type="click"
              onClick={onSearchSwpAnalysisPull}
        ...
        {searchingSchemes ? (
          <>
            <h5 className="mt-7 leading-none text-center text-xl mb-4 font-extrabold text-gray-900 dark:text-white pb-1">
              ...
              {inputCalculatorRules.formatDateToWords(investmentDate)} to{" "}
              {inputCalculatorRules.formatDateToWords(withdrawalDate)}
            </h5>
          </>
        ) : (
          <div
                        ...
                        >
                          ...
                          <h5 className="mt-2 leading-none text-center text-xl mb-4 font-extrabold text-gray-900 dark:text-white pb-1">
                            {`From ${inputCalculatorRules.formatDateToWords(
                              investmentDate
                            )}  to  ${inputCalculatorRules.formatDateToWords(
                              withdrawalDate
                            )}`}
                          </h5>
                        </div>
                          ...
                          <table className="w-full text-lg text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <thead className="text-md text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                              ...
                              <tr>
                                ...
                                <th scope="col" className="px-6 py-3">
                                  {inputCalculatorRules.formatSensex(returnVal) + '%'}
                                </th>
                              </tr>
                            </thead>
                          </table>
