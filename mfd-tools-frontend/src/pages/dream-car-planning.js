...
export default function dreamCarPlanning() {
  ...
  const [dreamCarAge, setDreamCarAge] = useState("");
  const [currentInvestmentAmt, setCurrentInvestmentAmt] = useState("");
  const [inflationRate, setInflationRate] = useState("");

  const [dreamCarFutureCost,setDreamCarFutureCost] = useState("");
  ...
  const onSubmitGoalInvestPlan = (e) => {
    ...
    if (
      currentAge.length > 0 &&
      dreamCarCost.length > 0 &&
      dreamCarAge.length > 0 &&
      returnRate.length > 0 &&
      currentInvestmentAmt.length > 0 &&
      inflationRate.length > 0 &&
      inputCalculatorRules.numericRegex.test(currentAge) &&
      inputCalculatorRules.numericRegex.test(dreamCarCost) &&
      inputCalculatorRules.numericRegex.test(dreamCarAge) &&
      ...
      mfToolsService
        .calculateGoalInvestPlan(
          ...
          Number(dreamCarAge.replace(/,/g, "")),
          Number(dreamCarCost.replace(/,/g, "")),
        ...
        .then((res) => {
          ...
          setDreamCarFutureCost(inputCalculatorRules.formatINR(res.data.futureCost));
    ...
    <>
      ...
      <div style={{ margin: "0 8%" }}>
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block mb-2 text-sm font-medium text-gray-700">
              Expected Car Purchase Age (Years)
            </label>
            <input
              value={dreamCarAge}
              onChange={onChangeDreamCarAge}
              type="text"
              min="0"
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                       focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
          ...
          <div>
            <label className="block mb-2 text-sm font-medium text-gray-700">
              Current Cost of Dream Car (₹)
            </label>
            <input
              value={dreamCarCost}
              onChange={onChangeDreamCarCost}
              type="text"
              min="0"
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                       focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
