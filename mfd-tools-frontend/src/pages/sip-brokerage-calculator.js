...
export default function simpleSipCalculator() {
  ...
  const [sipTenor, setSipTenor] = useState("");
  ...
  const onSubmitSipBrokerageCalculator = (e) => {
    e.preventDefault();
    if (
      sipAmount.length > 0 &&
      sipTenor.length > 0 &&
      ...
      inputCalculatorRules.numericRegex.test(sipTenor) &&
      ...
      mfToolsService
        .calculateSipBrokerage(
          ...
          Number(sipTenor.replace(/,/g, ""))
        )
        .then((res) => {
          console.log(res.data);
          if (res.data.totalCommission !== undefined) {
            setTotalCommission(
              inputCalculatorRules.formatINR(res.data.totalCommission)
            );
          }
  ...
  return (
    <>
      <h1 className="text-center font-semibold mb-10 text-3xl">
        Brokerage Calculation - SIP Business
      </h1>
