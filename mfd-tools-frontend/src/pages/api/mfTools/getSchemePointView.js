...
const getSchemePointView = async (req, res) => {
  ...
  if (req.method === "POST") {
    let { token, fundHouse, schemeType, previousDate, nextDate } = req.body;

    // Making a post request to hit our backend api-endpoint
    const apiRes = await fetch(`${API_URL}/api/mfTools/getSchemePointView`, {
      method: "POST",
      headers: {
...
export default getSchemePointView;
