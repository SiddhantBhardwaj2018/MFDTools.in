...
const getSchemeReturnsView = async (req, res) => {
  ...
  if (req.method === "POST") {
    let { token, fundHouse, schemeType, timePeriod, offset } = req.body;

    // Making a post request to hit our backend api-endpoint
    const apiRes = await fetch(`${API_URL}/api/mfTools/getSchemeReturnsView`, {
      method: "POST",
      headers: {
...
export default getSchemeReturnsView;
