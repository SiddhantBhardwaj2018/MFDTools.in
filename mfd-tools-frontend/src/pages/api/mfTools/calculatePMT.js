...
const calculatePmtValue = async (req, res) => {
  ...
  if (req.method === "POST") {
    let { token, rate, nper, pv, fv } = req.body;

    // Making a post request to hit our backend api-endpoint
    const apiRes = await fetch(`${API_URL}/api/mfTools/calculatePmtValue`, {
      method: "POST",
      headers: {
...
export default calculatePmtValue;
