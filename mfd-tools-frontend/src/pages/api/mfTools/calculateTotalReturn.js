...
const calculateTotalReturn = async (req, res) => {
  ...
  if (req.method === "POST") {
    let { token, initialLumpsum, principal, rate, years, inflation } = req.body;

    // Making a post request to hit our backend api-endpoint
    const apiRes = await fetch(
      `${API_URL}/api/mfTools/calculateTotalReturnWithLumpSumAndSIP`,
      {
        method: "POST",
        headers: {
