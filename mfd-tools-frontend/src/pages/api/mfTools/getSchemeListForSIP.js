...
const getSchemeListForSIP = async (req, res) => {
  ...
  if (req.method === "POST") {
    let { token, fundHouse, investDate, schemeType } = req.body;

    // Making a post request to hit our backend api-endpoint
    const apiRes = await fetch(`${API_URL}/api/mfTools/getSchemeListForSIP`, {
      method: "POST",
      headers: {
...
export default getSchemeListForSIP;
