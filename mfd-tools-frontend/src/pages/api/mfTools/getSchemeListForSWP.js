...
const getSchemeListForSWP = async (req, res) => {
  ...
  if (req.method === "POST") {
    let { token, fundHouse, investDate, schemeType, withdrawalDate } = req.body;

    // Making a post request to hit our backend api-endpoint
    const apiRes = await fetch(`${API_URL}/api/mfTools/getSchemeListForSWP`, {
      method: "POST",
      headers: {
...
export default getSchemeListForSWP;
