...
const distributorCommissionCalc = async (req, res) => {
  ...
  if (req.method === "POST") {
    let { token, sipAmt, sipRate, commissionRate, time } = req.body;

    // Making a post request to hit our backend api-endpoint
    const apiRes = await fetch(
      `${API_URL}/api/mfTools/distributorCommissionCalc`,
      {
        method: "POST",
        headers: {
...
export default distributorCommissionCalc;
