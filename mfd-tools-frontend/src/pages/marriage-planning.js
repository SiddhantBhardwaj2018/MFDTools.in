...
export default function marriagePlanning() {
  ...
  const [marriageFutureCost,setMarriageFutureCost] = useState("");
  ...
  const onSubmitGoalInvestPlan = (e) => {
    ...
    if (
      currentAge.length > 0 &&
      marriageCost.length > 0 &&
      marriageAge.length > 0 &&
      returnRate.length > 0 &&
      currentInvestmentAmt.length > 0 &&
      inflationRate.length > 0 &&
      inputCalculatorRules.numericRegex.test(currentAge) &&
      inputCalculatorRules.numericRegex.test(marriageCost) &&
      inputCalculatorRules.numericRegex.test(marriageAge) &&
      ...
      mfToolsService
        .calculateGoalInvestPlan(
          ...
          Number(marriageAge.replace(/,/g, "")),
          Number(marriageCost.replace(/,/g, "")),
        ...
        .then((res) => {
          ...
          setMarriageFutureCost(inputCalculatorRules.formatINR(res.data.futureCost));
    ...
    <>
      ...
      <div style={{ margin: "0 8%" }}>
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block mb-2 text-sm font-medium text-gray-700">
              Expected Marriage Age (Years)
            </label>
            <input
              value={marriageAge}
              onChange={onChangeMarriageAge}
              type="text"
              min="0"
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg 
                       focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                ...
                <thead className="text-md text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    <th scope="col" className="px-6 py-3">
                      Future Cost Of Child&apos;s Marriage
                    </th>
                    <th scope="col" className="px-6 py-3">
                      {marriageFutureCost}
                    </th>
